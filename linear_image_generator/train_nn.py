...
def build_generator(noise_dim, piece_size=(50, 50)):
    ...
    inputs_images = layers.Input(shape=(3, *piece_size, 3))

    noise_transform = layers.Dense(50 * 50 * 8)(inputs_noise)
    noise_transform = layers.LeakyReLU(negative_slope=0.2)(noise_transform)
    noise_transform = layers.Reshape((*piece_size, 8))(noise_transform)
    noise_transform = layers.Conv2DTranspose(8, (16, 16), padding='same')(noise_transform)
    noise_transform = layers.LeakyReLU(negative_slope=0.2)(noise_transform)
    
    inputs_image_transform = layers.ConvLSTM2D(8, (16, 16), padding='same',
                                               return_sequences=True)(inputs_images)
    inputs_image_transform = layers.ConvLSTM2D(8, (8, 8), padding='same')(inputs_image_transform)

    output = layers.Concatenate(axis=-1)([inputs_image_transform, noise_transform])
    output = layers.Conv2DTranspose(4, (4, 4), padding="same")(output)
    output = layers.LeakyReLU(negative_slope=0.2)(output)
    output = layers.Conv2DTranspose(3, (7, 7), padding="same", activation="sigmoid")(output)

    model = models.Model([inputs_images, inputs_noise], output, name="Generator")
    return model


def build_discriminator(piece_size=(50, 50)):
    input_image = layers.Input(shape=(3, *piece_size, 3))
    condition_image = layers.Input(shape=(*piece_size, 3))
    condition_image_r = layers.Reshape((1, *piece_size, 3))(condition_image)
    x = layers.Concatenate(axis=1)([input_image, condition_image_r])
    x = layers.Conv3D(64, (3, 3, 12), activation="relu")(x)
    x = layers.Flatten()(x)
    x = layers.Dense(1, activation="sigmoid")(x)
    
    model = models.Model([input_image, condition_image], x, name="Discriminator")
    return model
...
if __name__ == "__main__":
    ...
    imag_gan.save_model('models/first_linear_image_generator')
