...
class SheetImageGenerator(tf.keras.utils.Sequence):
    def __init__(self, directory, batch_size=16, piece_size=(50, 50), pises_count=4):
        self.directory = directory
        self.batch_size = batch_size
        self.piece_size = np.array(piece_size)
        self.files = sorted(os.listdir(directory))[:1000]
    ...
    def __getitem__(self, item):
        file_pash = os.path.join(self.directory, self.files[item])  # self.files[item]  '00000000.jpg'
        ...
        for _ in range(self.batch_size):
            vector = np.random.choice([self.get_vertical_line, self.get_horizontal_line])
        ...
        return np.array(batch_input) / 255, np.array(batch_output) / 255
...
if __name__ == '__main__':
    gen = SheetImageGenerator('data2', batch_size=10, piece_size=(200, 200))
    x, y = gen[0]
    print(x.shape)
    for i in range(5):
        plt.figure()
